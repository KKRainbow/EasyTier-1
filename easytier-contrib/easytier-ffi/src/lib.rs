...
//! - `get_error_msg`: copy the last thread-local error message out of the FFI layer.
...
/// Return the last FFI error message for the calling thread.
///
/// The error buffer is thread-local. Call this on the same thread that received
/// `-1` or `0` from another API. If there is no error message, this writes a
/// null pointer to `out`.
///
