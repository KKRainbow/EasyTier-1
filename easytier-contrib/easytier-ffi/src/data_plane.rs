...
#[cfg(feature = "ffi-dataplane")]
static DATA_PLANE_USAGE_LOCK: once_cell::sync::Lazy<Mutex<()>> =
...
#[cfg(feature = "ffi-dataplane")]
fn enter_data_plane_handle_creation() -> Option<std::sync::MutexGuard<'static, ()>> {
    if data_plane_rejected() {
        return None;
    }

    let guard = match DATA_PLANE_USAGE_LOCK.lock() {
        Ok(guard) => guard,
        Err(err) => {
...
#[cfg(feature = "ffi-dataplane")]
pub(crate) fn lock_for_config_server_start() -> Result<std::sync::MutexGuard<'static, ()>, String> {
...
pub(crate) unsafe fn data_plane_tcp_connect(
    ...
    let _data_plane_usage_guard = match enter_data_plane_handle_creation() {
        Some(guard) => guard,
        None => return 0,
    };
