...
impl Drop for StartedInstanceCleanup {
    fn drop(&mut self) {
        if self.armed {
            let _ = self.manager.delete_network_instance(vec![self.inst_id]);
...
impl WebClientService for InstanceManageRpcService {
    ...
    async fn run_network_instance(
        ...
        let mut started_instance =
            StartedInstanceCleanup::new(self.manager.clone(), effective_id, cleanup_config_file);
